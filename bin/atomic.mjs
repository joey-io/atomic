...
async function main() {
  ...
  const atomic = await createAtomic({
    ...config,
    models: [...(config.models ?? []), ...modelSources],
    modelDirectory: flags['model-dir'] === false
      ? false
      : flags['model-dir'] ?? config.modelDirectory
  });
  ...
  switch (command) {
    case 'observe': {
      ...
      result = await atomic.observe(input, operationOptions(flags));
      break;
    }
    case 'parse': {
      const input = await readInput(positional[0]);
      result = await atomic.parse(input, {
        ...operationOptions(flags),
        persist: flags.persist !== false,
        accept: flags.accept === 'all' ? 'all' : undefined
      });
      break;
    }
    case 'resolve': {
      const candidate = await readJsonInput(positional[0]);
      result = await atomic.resolve(candidate, operationOptions(flags));
      break;
    }
    case 'fold': {
      const candidate = await readJsonInput(positional[0]);
      result = await atomic.fold(candidate, operationOptions(flags));
      break;
    }
    case 'models': {
      const name = positional[0];
      result = name
        ? atomic.models.get(name)
        : atomic.models.list().map(model => ({
            name: model.name,
            extends: model.extends,
            description: model.description,
            source: atomic.models.source(model.name)
          }));
      if (name && !result) throw new Error(`Unknown model "${name}"`);
      break;
    }
    default:
      throw new Error(`Unknown command "${command}". Run atomic help.`);
  }
...
function parseArgs(argv) {
  ...
  while (args.length) {
    ...
    if (key === 'model') flags.model = [...arrayFlag(flags.model), value];
...
async function readInput(file) {
  if (!file || file === '-') {
    ...
    return { name: 'stdin.txt', contentType: 'text/plain', text: bytes.toString('utf8') };
  }
...
function operationOptions(flags) {
  let context = {};
  if (flags.context) {
    try { context = JSON.parse(flags.context); }
    catch (error) { throw new Error(`--context must be JSON: ${error.message}`); }
  }
  return {
...
function writeResult(result, flags) {
  if (flags.jsonl && Array.isArray(result)) {
