...
for (let i = 0; i < 30; i++) {
  const adv = (i % 12) + 1, d = ((adv - 1) % 5) + 1;
  await atom('story', `b-st${i + 1}`, T,
    { title: `Why this matters #${i + 1}`, body: `An advocate shared a first-hand story about the impact in their district (#${i + 1}).`, advocate: A(`b-adv${adv}`), district: A(`b-d${d}`) },
