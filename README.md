...
- **`attr`-field indexes.** Reads are scoped by tenant (`shard`) and type (`model`) via SQL indexes (`atom_by_shard`, `atom_by_model`) — `getStore`, `listModel`, identity dedup, inverse-edge resolution, and the token scans all hit an index rather than the whole store. Indexes over arbitrary `attr` *fields* (e.g. `attr->>'$.state'` via generated columns) would make filtered/sorted queries on a single model index-backed too; today those filter in JS over the (already tenant- and model-scoped) set. This is a performance optimization, not a missing feature — and it is **mutually exclusive with encryption at rest**: a generated column must read the JSON body, which is opaque ciphertext when `ATOMIC_KEY` is set. So it stays an explicit, opt-in boundary rather than a default.
