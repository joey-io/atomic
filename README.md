...
**What works today (all exercised by `node test.mjs`, 82 assertions):**
...
- **CSV import / export, generated from the model.** Every model table and index result has an **export CSV** link (`?as=csv`; respects read grants and redaction), every model offers a **download template** (`?as=template` — header row of `id, manifest, <fields>`), and the generated form's method picker gains an **IMPORT** option that reveals a drag-and-drop dropzone. Import is not a new verb: a `POST` to a model with a `text/csv` body (or a JSON array) is a bulk create — each row runs through the normal `create()` path (grants, attenuation, rules, dedup, hooks) and the response is a per-row `{ imported, failed }` summary.
...
- **`attr`-field indexes.** Reads are scoped by tenant (`shard`) and type (`model`) via SQL indexes (`atom_by_shard`, `atom_by_model`) — `listModel`, `runIndex` (a stored index over a single model), identity dedup, inverse-edge resolution, and the token scans all hit an index rather than materializing the whole store. Indexes over arbitrary `attr` *fields* (e.g. `attr->>'$.state'` via generated columns) would make filtered/sorted queries on a single model index-backed too; today those filter in JS over the (already tenant- and model-scoped) set. This is a performance optimization, not a missing feature — and it is **mutually exclusive with encryption at rest**: a generated column must read the JSON body, which is opaque ciphertext when `ATOMIC_KEY` is set. So it stays an explicit, opt-in boundary rather than a default.
...
Use `atom://` when the target is a record that exists on its own and may be shared (a company, a deal). Use `embed://` for a field group that belongs to one parent and is not shared (an address).
