...
- Reads are now scoped by tenant (`shard`) and type (`model`) via SQL indexes (`atom_by_shard`, `atom_by_model`) — `getStore`, `listModel`, identity dedup, inverse-edge resolution, and the token scans all hit an index rather than the whole store. **Still TODO:** indexes over arbitrary `attr` *fields* (e.g. `attr->>'$.state'` via generated columns), which would make filtered/sorted queries on a single model index-backed too; today those filter in JS over the model-scoped set.
